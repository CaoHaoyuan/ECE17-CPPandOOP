...
namespace ECE17 {
  ...
  protected:
    std::string encodeWord(std::string anInput,int &counter);
    std::string decodeWord(std::string anInput);
    ...
    std::map<std::string,int> encodeDict; // this dictionary should not be used in decompression!
    std::vector<std::string> decodeDict;  // this vector should not be used in compression!
  };
